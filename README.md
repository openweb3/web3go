...
`SendTransactionByArgs` also supports EIP-7702 transactions via `AuthorizationList`.

- If an authorization entry is unsigned (`V=0`, `R=0`, `S=0`), signable provider will sign it automatically before sending.
- You can call `SetCodeAuthorization.Authority()` to get the authority address recovered from the signed authorization.

```golang
chainId, _ := c.Eth.ChainId()
nonce, _ := c.Eth.TransactionCount(from, nil)

auth := ethrpctypes.SetCodeAuthorization{	
	ChainID: *uint256.NewInt(*chainId),
	Address: common.HexToAddress("0x7d682e65EFC5C13Bf4E394B8f376C48e6baE0355"),
	Nonce:   nonce.Uint64() + 1,
}

txHash, err := c.Eth.SendTransactionByArgs(types.TransactionArgs{
	From:              &from,
	To:                &to,
	AuthorizationList: []ethrpctypes.SetCodeAuthorization{auth}, // unsigned tuple is acceptable
})
if err != nil {
	panic(err)
}
fmt.Println("7702 tx hash:", txHash)
```
